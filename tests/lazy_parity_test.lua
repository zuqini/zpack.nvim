...
describe("virtual = true (zpack_nvim-fqt)", function()
  before_each(helpers.setup_test_env)
  after_each(helpers.cleanup_test_env)

  it("virtual plugins are NOT registered with vim.pack but still run config", function()
      ...
      spec = {
        {
          'meta/virtual-plugin',
          virtual = true,
          config = function() config_ran = true end,
        },
      },
      defaults = { confirm = false },
    })
    ...
    -- The virtual plugin must NOT be in any vim.pack.add call.
    local seen_in_pack_add = false
    for _, call in ipairs(_G.test_state.vim_pack_calls) do
      for _, pack_spec in ipairs(call) do
        if pack_spec.src == 'https://github.com/meta/virtual-plugin' then
          seen_in_pack_add = true
        end
      end
    end
    assert.is_false(seen_in_pack_add,
      "virtual = true must skip vim.pack.add registration")

    -- But its config function must still run at startup.
    assert.is_true(config_ran,
      "virtual plugin config must still run (the meta-plugin's purpose)")
  end)

  it("dependencies of virtual plugins are still installed", function()
    require('zpack').setup({
      spec = {
        {
          'meta/wrapper',
          virtual = true,
          dependencies = { 'test/real-dep' },
        },
      },
      defaults = { confirm = false },
    })
    ...
    local state = require('zpack.state')
    assert.is_not_nil(state.spec_registry['https://github.com/test/real-dep'],
      "Dependencies of virtual plugins must still install")

    -- Confirm the dep WAS registered with vim.pack
    local seen_dep = false
    for _, call in ipairs(_G.test_state.vim_pack_calls) do
      for _, pack_spec in ipairs(call) do
        if pack_spec.src == 'https://github.com/test/real-dep' then
          seen_dep = true
        end
      end
    end
    assert.is_true(seen_dep, "Dep of virtual plugin must reach vim.pack.add")
  end)
end)
